---
title: "OpenChain AI SBOM 컴플라이언스 관리 가이드: AI 공급망 컴플라이언스 프로그램의 최소 요구사항"
linkTitle: "OpenChain AI SBOM 컴플라이언스 가이드"
date: 2026-06-12
weight: 10
type: docs
description: >-
  리눅스 재단 산하 오픈체인 프로젝트의 AI 워크그룹이 작성한 AI SBOM 컴플라이언스 관리
  가이드를 1차 출처로 분석한다. ISO/IEC 5230 방법론을 AI 공급망으로 확장해 컴플라이언스
  프로그램이 갖춰야 할 최소 요구사항을 정의한 문서의 구조와 요구사항, 규제 동향, 의의와
  한계를 다룬다.
author: 장학성
categories: ["research"]
tags: ["AI SBOM", "OpenChain", "ISO/IEC 5230", "ISO/IEC 42001", "오픈소스 컴플라이언스", "AI BOM", "공급망"]
ai_generated: true
...
source_pdf: "1차 출처 — OpenChain AI-WG RFC 원문, ISO/IEC 5230·42001·5962, IETF RFC 2119, Regulation (EU) 2024/1689, OECD HAIP 보고 프레임워크"
---
...
여기서 초안의 결함 하나를 짚어야 합니다. 원본의 목차(Table of Contents)와 본문의 절 번호가 어긋납니다. 목차에는 3.9 "AI content review and approval"(AI 콘텐츠 검토 및 승인), 3.10 "AI System Bill of Materials", 3.11 "Governance"가 있으나, 본문에는 "AI content review and approval" 절이 누락되거나 다른 절에 병합되어 존재하지 않습니다<a id="a1-ref-11"></a>[A1](#a1). 그 결과 본문은 해당 항목 이후를 한 칸씩 당겨 3.9 "AI System Bill of Materials", 3.10 "Governance"로 번호를 매깁니다. 이 보고서의 절 번호와 표 1은 모두 본문 기준을 따릅니다. 목차 기준으로는 요구사항이 열한 개(누락된 "AI 콘텐츠 검토 및 승인" 포함)이고, 본문 기준으로는 열 개인 셈입니다. 살아있는 문서 단계의 편집 미비로 보이며, 정식 Version 1에서 정리되었을 가능성이 있으나 이 보고서는 초안 스냅샷을 대상으로 하므로 확인 범위 밖입니다.
...
## 6. 기존 AI BOM 보고서와의 관계

이 가이드는 같은 워크스페이스의 별도 연구보고서 *AI BOM 연구 및 실무 적용 사례*와 상호 보완 관계에 있습니다. 그 보고서의 III장은 EU 인공지능법(Article 11·Annex IV, Article 53과 Annex XI·XII), EU 사이버 복원력법(Cyber Resilience Act, CRA), 한국 AI기본법 등의 조문 단위 문서화 의무를 AI BOM의 여섯 필드군(시스템 구성, 모델 정보, 데이터셋 정보, 인프라와 자원, 관리 정보, 위험과 거버넌스)에 매핑해, 규제가 실제로 요구하는 기재 항목을 도출합니다. 두 문서는 층위가 다릅니다. OpenChain 가이드는 "컴플라이언스 프로그램이 갖춰야 할 최소 요구사항"을 프로세스 관점에서 정의하고(정책, 역량, 자원, SBOM 절차의 존재 여부), AI BOM 보고서는 "그 프로그램이 산출할 AI BOM에 무엇을 담아야 규제 의무를 충족하는가"를 규제 매핑으로 구체화합니다. 가이드 3.5, 3.6, 3.9절이 추상적으로 요구하는 라이선스와 투명성, SBOM 절차에 AI BOM 보고서 III장의 조문별 필드 매핑을 결합하면, 프로세스 요구와 데이터 항목 요구가 한 체계로 이어집니다.
