...
소프트웨어 구성 분석(Software Composition Analysis, SCA)은 코드에 들어온 오픈소스를 식별하고 그 취약점과 라이선스를 관리하는 활동 전반을 가리킵니다. 대부분의 SCA는 위 표의 첫 줄처럼 선언된 의존성을 봅니다. 스니펫 단위 매칭은 그중 일부 상용 도구만 갖춘 별도 기능으로, 소스 코드 본문을 수많은 오픈소스 프로젝트와 대조해 패키지로 선언되지 않은 조각의 출처를 찾습니다. 여기서 다루는 것은 SCA 전체가 아니라 이 스니펫 매칭이라는 좁은 기능입니다.
...
위 네 가지와 이런 운영 방식을 자기 회사 상황에 맞춰 보면, 스니펫 검사에 얼마나 비중을 둘지에 대한 답은 회사마다 다르게 나올 것입니다.
...
다만 보안에서 스니펫 스캔이 주력은 아닙니다. 도구마다 역할이 다릅니다.
...
- **스니펫과 클론 스캔** — 라이브러리로 선언되지 않은 채 복사돼 들어온 취약 코드를 잡는 좁은 보조입니다. 취약점이 복사를 통해 번지고 상류가 고쳐져도 복사본은 남는다는 점이 연구로 확인됐지만 <a id="d3-ref-1"></a>[D3](#d3), 이걸 잡으려면 라이선스 출처용 스니펫 기능이 아니라 알려진 취약 함수와 대조하는 별도 기법이 필요합니다.

정리하면, AI 코드의 보안은 SAST와 의존성 단위 SCA가 맡습니다. 스니펫 스캔은 보안에서 좁은 틈을 메우는 보조이고, 라이선스용 스니펫 기능을 보안 대책으로 오인해서는 안 됩니다.
...
<a id="d3"></a>**D3.** Kim, S., Woo, S., Lee, H., & Oh, H. (2017). *VUDDY: A Scalable Approach for Vulnerable Code Clone Discovery*. IEEE S&P 2017. <https://seulbae-security.github.io/pubs/vuddy-sp17.pdf> (접속: 2026-06-08). — 취약점이 코드 클론을 통해 전파되고 상류 패치 후에도 복사본이 미패치로 남는 문제와 그 탐지. <a href="#d3-ref-1" onclick="event.preventDefault(); history.back(); return false;" title="본문으로 돌아가기">↩</a>
