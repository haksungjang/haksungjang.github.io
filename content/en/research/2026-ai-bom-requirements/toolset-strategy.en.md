---
title: "AI BOM Toolset Design Strategy"
linkTitle: "Toolset Strategy"
date: 2026-06-29
weight: 4
type: docs
description: >-
  Surveys seven tool categories through their official repositories and documentation to
  lay out what to reuse, extend, or build new; the build order; the policy schema for
  codifying the matrix; and the Dependency-Track integration architecture.
author: Haksung Jang
categories: ["research"]
...
A table mapping each of the G7's 50 elements to a CycloneDX path (and, eventually, an SPDX path) is kept in one place. The [AI BOM Field Requirements Matrix]({{< relref "_index.md" >}}) already contains an element-by-element source mapping, so moving it into a machine-readable form looks like this.
