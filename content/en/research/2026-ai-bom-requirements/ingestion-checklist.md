---
title: "외부 모델과 데이터셋 도입 점검 체크리스트"
linkTitle: "도입 점검 체크리스트"
date: 2026-06-29
weight: 3
type: docs
description: >-
  외부 AI 모델이나 데이터셋을 가져와 활용할 때 AI BOM을 근거로 투명성과 위험을 점검하는
  체크리스트입니다. 식별, 라이선스, 데이터 적법성, 보안 위험을 단계별로 확인합니다.
author: 장학성
categories: ["research"]
...
이 체크리스트는 자사 개발팀이 외부 AI 모델이나 데이터셋을 가져와 활용할 때, AI를 위한 자재 명세서(AI Bill of Materials, AI BOM)를 근거로 투명성과 위험을 점검하는 기준입니다. 확인 항목은 [「AI BOM 필드 요구사항 매트릭스」]({{< relref "_index.md" >}})의 도입 열에서 도출했습니다.
...
![필수 확인 다섯 단계를 차례로 거친 뒤 도입 가능과 도입 보류로 갈리는 흐름. 라이선스 충돌, 출처 불명확, 민감 데이터 미표기, 수용 불가 취약점 넷 중 하나라도 해당하면 보류](./ingestion-decision.png)
