---
title: "기업용 AI BOM 필드 요구사항 매트릭스 — 표준과 규제 근거로 정의한 필수/선택"
linkTitle: "AI BOM 필드 요구사항"
date: 2026-06-29
weight: 10
type: docs
description: >-
  G7 「AI를 위한 SBOM 최소 요소」의 50개 요소를 SPDX 3.0.1, CycloneDX 1.6, NTIA 2021,
  OpenChain AI V1 등 권위 있는 표준과 CRA·AI Act·FDA 규제 근거로 대조해 AI BOM 필드의
  필수와 선택을 판정합니다. 같은 매트릭스를 생산·도입·공급사 세 맥락에 적용하는 운영 문서와
  도구 세트 전략까지 다섯 부분으로 정리한 시리즈입니다.
author: 장학성
categories: ["research"]
...
기업의 오픈소스 관리 체계는 이미 공급사에 소프트웨어 자재 명세서(Software Bill of Materials, SBOM) 제출을 의무화하고 제출 요구사항을 운영하는 단계에 와 있습니다. AI 시스템은 같은 투명성 요구를 모델과 데이터셋까지 확장해야 하지만, 기존 SBOM 요구사항은 소프트웨어 구성요소만 다루므로 AI 고유 정보를 담지 못합니다. 이 보고서는 AI를 위한 자재 명세서(이하 AI BOM)에 어떤 필드를 필수로, 어떤 필드를 선택으로 요구할지를 국제 표준과 규제 근거로 정의합니다.

AI BOM을 통칭으로 삼은 이유를 밝혀 둡니다. 대조 대상 가운데 셋이 같은 대상을 서로 다른 이름으로 부르기 때문입니다. 필드 목록을 가져온 G7 사이버보안 작업반은 SBOM for AI, OpenChain 프로젝트는 AI SBOM, CycloneDX는 기계학습 자재 명세서(Machine Learning Bill of Materials, ML-BOM)를 씁니다. 나머지 둘 가운데 SPDX는 별도 약어 대신 AI 프로파일로 표현하고, NTIA 최소 요소는 AI 이전의 일반 SBOM을 다룹니다. 어느 하나를 대표로 세우면 나머지 표준을 지칭할 때 어긋나므로, 아래에서는 일반 개념을 AI BOM으로 부르고 개별 표준을 지칭할 때만 그 표준의 명칭을 그대로 옮깁니다.
...
![합의 판정을 기준으로 세 역할이 각자 필수의 경계를 다시 긋는 구조. 생산은 합의 필수 20을 그대로 두고, 도입은 문서 메타데이터를 내리는 대신 위험 판단 필드 셋을 올려 15개, 공급사는 둘을 올려 20개를 필수로 둔다](./role-scope.png)

**그림 1.** 같은 매트릭스를 두고 역할마다 달라지는 필수의 경계 *(조사 종합)*
...
- NTIA 「소프트웨어 자재 명세서 최소 요소」(2021): 일반 소프트웨어 SBOM의 일곱 개 기준 데이터 필드를 규정합니다.
...
![다섯 출처 가운데 G7은 모든 요소에 언제나 1표를 주므로, 실제 판정은 SPDX·CycloneDX·NTIA·OpenChain 네 곳 중 한 곳이라도 요구하는지로 귀결된다](./consensus-rule.png)

**그림 2.** 합의 판정 규칙과 G7의 위치 *(조사 종합)*

이 규칙을 적용한 결과, 50개 중 20개가 필수, 30개가 선택입니다.

### 2.3 규제 근거 표시

구속력 있는 규제가 해당 필드에 적용되는지는 별도 축으로 표시합니다. 합의 집계에는 넣지 않습니다. 어떤 규제도 AI BOM을 그 이름으로 강제하지 않기 때문입니다. 사이버 복원력법(Cyber Resilience Act, CRA)은 일반 소프트웨어 SBOM을, 인공지능법(AI Act)과 미국 식품의약국(Food and Drug Administration, FDA) 가이던스, 국내 제도는 문서화 의무를 요구합니다. `직접`은 규제가 그 항목을 명시 요구하는 경우, `간접`은 SBOM 항목은 아니나 문서화·취약점 처리 의무가 같은 정보를 사실상 요구하는 경우입니다.
...
![클러스터별 필수와 선택 분포 가로 막대. 메타데이터·시스템·모델·데이터셋에는 필수가 있으나 인프라·보안 속성·핵심성과지표는 필수가 하나도 없다](./cluster-distribution.png)

**그림 3.** 클러스터별 필수·선택 분포 *(조사 종합)*
...
- [공급사 AI BOM 제출 요구사항]({{< relref "supplier-requirements.md" >}}) — 공급사에 제출을 요구할 필드와 제출 규약
- [자사 개발팀 AI BOM 작성 지침]({{< relref "producer-guide.md" >}}) — 자사 생산팀이 채워야 하는 필수·권장 항목
- [외부 모델과 데이터셋 도입 점검 체크리스트]({{< relref "ingestion-checklist.md" >}}) — 외부 모델·데이터셋을 들여올 때의 위험 점검
- [AI BOM 도구 세트 설계 전략]({{< relref "toolset-strategy.md" >}}) — 매트릭스를 정책으로 코드화하고 기존 도구와 엮는 전략
